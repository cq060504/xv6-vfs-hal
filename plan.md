...
先看清楚原版有哪些平台相关文件（这些都是你要"包裹"进 HAL 的）：

```
kernel/
  entry.S          ← 启动汇编，完全平台相关
  trampoline.S     ← 用户态陷入，寄存器操作，平台相关
  swtch.S          ← 上下文切换汇编，平台相关
  trap.c           ← 中断/异常处理，调用 PLIC，平台相关
  plic.c           ← RISC-V 专用中断控制器
  virtio_disk.c    ← 设备驱动，相对平台无关
  vm.c             ← 页表管理，sv39 格式，平台相关
  proc.c           ← 部分寄存器操作
  riscv.h          ← 大量 RISC-V CSR 寄存器定义
  memlayout.h      ← 内存地址布局，平台相关
```
