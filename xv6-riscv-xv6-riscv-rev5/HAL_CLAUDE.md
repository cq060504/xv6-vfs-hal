...
## 任务二：FAT32 文件系统实现（RISC-V only）

### 约束条件
1. **仅 RISC-V 架构**（`ARCH=riscv`），不涉及 LoongArch
2. **不影响现有 VFS**：xv6fs 和 ext2 的现有功能不退化
3. **尽量精简**：只实现基本读写框架，可运行简单测试样例
4. **不修改 ext2test1/2/3.c** 等已有测试

### 技术文档
- `Fat-技术文档.md`（项目根目录）：FAT32 规范中文译版，包含 BPB、FAT 表、目录项、簇链等核心结构

### 实现计划

#### 第一步：理解现有模式（花 0.5h）
- [ ] 阅读 `kernel/ext2.c` + `kernel/ext2.h`：理解 VFS 后端注册模式（`vnode_ops` + `vfs_ops` + `*_mount`）
- [ ] 阅读 `kernel/xv6fs.c`：确认不修改此文件
- [ ] 阅读 `kernel/vfs.c` + `kernel/vfs.h`：理解 `vfs_register` 如何工作

#### 第二步：创建 fat32 磁盘镜像（Makefile）
- [ ] 在 `Makefile` 中添加 `fat32.img` 构建目标（`dd` + `mkfs.vfat -F 32`）
- [ ] RISC-V QEMU 添加第三个 virtio 磁盘（`bus=virtio-mmio-bus.2`）
- [ ] `VIRTIO_NDISK` 扩展为 3（RISC-V only）

#### 第三步：FAT32 数据结构定义（`kernel/fat32.h`）
- [ ] BPB（BIOS Parameter Block）结构体
- [ ] FAT32 扩展 BPB 字段（`BPB_FATSz32`, `BPB_RootClus`, `BPB_FSInfo` 等）
- [ ] 短目录项结构（`DIR_Name[11]`, `DIR_Attr`, `DIR_FstClusLO/HI`, `DIR_FileSize`）
- [ ] FAT 表项操作宏（EOC 标记 `0x0FFFFFF8`，坏簇 `0x0FFFFFF7`）

#### 第四步：FAT32 VFS 后端实现（`kernel/fat32.c`）
精简实现，仅覆盖以下 `vnode_ops`：
- [ ] `fat32_mount()`：读取 BPB，验证 FAT32 签名，初始化 mount_priv
- [ ] `fat32_lookup()`：在目录中查找文件名
- [ ] `fat32_read()`：读取文件数据（遍历 FAT 簇链）
- [ ] `fat32_write()`：写入文件数据
- [ ] `fat32_create()`：创建新文件（分配目录项 + FAT 簇）
- [ ] `fat32_readdir()`：读取目录项
- [ ] `fat32_stat()`：返回文件 stat 信息
- [ ] `fat32_unlink()`：删除文件
- [ ] `fat32_root()`：返回根目录 vnode
- [ ] `fat32_destroy()`：释放 vnode 资源

核心 helper 函数：
- [ ] `fat32_cluster_to_sector(cluster)`：簇号 → 扇区号
- [ ] `fat32_read_fat(dev, cluster)`：读取 FAT 表中某簇的下一簇号
- [ ] `fat32_write_fat(dev, cluster, value)`：写入 FAT 表项
- [ ] `fat32_alloc_cluster(dev)`：分配一个空闲簇（bitmap/free count）
- [ ] `fat32_dir_lookup(dir, name)`：在目录中按文件名查找

#### 第五步：注册到 VFS + 挂载
- [ ] `vfs_init()` 中 `vfs_register(fat32_mount, "fat32")`
- [ ] `user/init.c` 中 `mkdir /fat` + `mount /fat 3 fat32`（可选，也可手动测试）
- [ ] 创建简单的测试程序 `user/fat32test.c`

#### 第六步：测试
- [ ] `make qemu` 启动，shell 中 `mount /fat 3 fat32`
- [ ] `echo hello > /fat/test.txt`
- [ ] `cat /fat/test.txt`
- [ ] `mkdir /fat/subdir`
- [ ] 基本读写、创建、删除文件验证

### 关键技术点（来自 Fat-技术文档.md）

| 概念 | 说明 |
|------|------|
| **扇区大小** | 通常 512 bytes（`BPB_BytsPerSec`） |
| **簇大小** | `BPB_SecPerClus` 个扇区，必须是 2 的幂，每簇 ≤ 32KB |
| **保留区** | `BPB_RsvdSecCnt` 个扇区，FAT32 典型值为 32 |
| **FAT 表** | 每个表项 32-bit（实际用 28-bit），高 4 位保留 |
| **簇 2** | 第一个数据簇，根目录起始簇号（`BPB_RootClus`，通常为 2） |
| **EOC 标记** | FAT32 用 `>= 0x0FFFFFF8` 表示簇链结束 |
| **坏簇标记** | `0x0FFFFFF7` |
| **目录项** | 32 bytes：`DIR_Name[11]` + `DIR_Attr` + `DIR_FstClusHI/LO` + `DIR_FileSize` |
| **文件名** | 8.3 格式，空格填充，不允许小写 |
| **卷标** | `ATTR_VOLUME_ID (0x08)`，根目录中唯一 |
| **FirstSectorofCluster(N)** | `= ((N - 2) * BPB_SecPerClus) + FirstDataSector` |
| **FirstDataSector** | `= BPB_RsvdSecCnt + (BPB_NumFATs * FATSz)`（FAT32 无根目录区） |
| **启动扇区** | 扇区 0，offset 510-511 为 `0xAA55` |
| **FSInfo 扇区** | FAT32 特有，扇区 1，含 `FSI_Free_Count` |
...
[ 扇区 0: BPB/启动扇区 ]
[ 扇区 1: FSInfo         ]
[ 保留扇区 ...           ]  (共 BPB_RsvdSecCnt 个，通常 32)
[ FAT 表 1               ]  (BPB_FATSz32 个扇区)
[ FAT 表 2 (备份)        ]
[ 数据区: 簇 2 = 根目录   ]
```

### 已验证结果
- RISC-V: `fat32test` ALL PASSED（6 项测试）
- LoongArch: 未适配（ROM 大小限制）

---

## 任务三：LoongArch FAT32 适配

### 约束条件
1. LoongArch 无 virtio 磁盘，使用 ramdisk（`objcopy` 嵌入镜像到内核）
2. QEMU LoongArch virt 的 `-bios` ROM 加载限制约 4MB
3. 当前 kernel.bin = 3.2MB（xv6 2MB + ext2 1MB + kernel）
4. 不影响 RISC-V 已有的三磁盘架构

### 核心难点：ROM 大小限制

```
当前: fs.img = xv6(2MB) + ext2(1MB) = 3MB → kernel.bin ≈ 3.2MB
加 fat32: fs.img = xv6(2MB) + ext2(1MB) + fat32(1MB) = 4MB → kernel.bin ≈ 4.2MB > 4MB ROM 限制
```

### 候选方案

#### 方案 A: 超小镜像 + 单 ramdisk（NDISK=1）

将 fat32 追加到 fs.img，以 fs_offset 方式访问：

```
fs.img = xv6(2MB) + ext2(512KB) + fat32(512KB) = 3MB → kernel.bin ≈ 3.2MB ✓
```

- fat32.h: 新增 `fs_offset` 字段
- fat32_mount: NDISK=1 时所有扇区偏移 `fs_offset/512` 个扇区
- 风险: ext2 512KB 可能不够（ext2test2 需写文件），fat32 512KB 勉强够基本测试

#### 方案 B: 双 ramdisk（NDISK=2）

```
ramdisk[0] = xv6(2MB)               → dev=1
ramdisk[1] = fat32_img.o(1MB)       → dev=2
ext2 追加到 fs.img → kernel.bin = 2MB + 1MB + 0.5MB = 3.5MB ✓
```

- ext2 在 ramdisk[0] 内以 fs_offset 方式访问（dev=1）
- fat32 独立 ramdisk（dev=2）
- 优点: ext2 和 fat32 完全独立，架构清晰
- 缺点: 需修改 LOONGARCH_FS_OBJS 使用 fat32_img.o 替代 fs1_img.o

#### 方案 C: 三 ramdisk + 超小镜像（NDISK=3）

```
ramdisk[0] = xv6(2MB)               → dev=1
ramdisk[1] = ext2_img.o(512KB)      → dev=2
ramdisk[2] = fat32_img.o(512KB)     → dev=3
kernel.bin = 2MB + 0.5MB + 0.5MB + 0.5MB = 3.5MB ✓
```

- 架构与 RISC-V 完全一致（NDISK=3，dev=1/2/3）
- 缺点: 实现最复杂，需修改 LoongArch hal_virtio.c 支持 3 个 ramdisk

### 推荐方案

**方案 B**（双 ramdisk），理由：
- kernel.bin 3.5MB 低于 ROM 限制
- 只需新增 1 个 ramdisk（RISC-V 已有 NDISK=3 的基础）
- ext2 保持现有 NDISK=1 的 fs_offset 方式不变
- fat32 完全独立磁盘与 RISC-V 架构一致

### 实现步骤（方案 B）

1. Makefile: LoongArch NDISK=2
2. Makefile: fat32_img.o 嵌入规则（替代 fs1_img.o）
3. hal/loongarch/hal_virtio.c: 第二个 ramdisk 使用 _binary_fat32_img_start
4. ext2.c: LoongArch NDISK=2 但 dev=1 + fs_offset（保持现有逻辑）
